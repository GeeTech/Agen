...
internal static class Program
{
    ...
    private static readonly LLMPlanner PlannerV2 = new LLMPlanner();
    ...
    private static MemoryManager BuildMemoryManager()
    {
        return new MemoryManager(
            new ShortTermMemory(capacity: 14),
            new LongTermMemory(),
            new Retriever(),
            new MemoryCompressionUnit(),
            compressionTurnThreshold: 10,
            compressionTokenThreshold: 2000);
    }
    ...
    private static Dictionary<string, string> LegacyParameterExtractor(string description, int stepId)
    {
        ...
        if (description.Contains("总结", StringComparison.Ordinal))
        {
            parameters["text"] = stepId > 1 ? $"$step{stepId - 1}" : description;
        }
    ...
    private static string ExecuteStep(PlanStep step, ToolRegistry registry, Dictionary<int, string> context)
    {
        ...
        if (tool is null)
        {
            return $"[Step {step.Id}] Executed: {step.Description}";
        }

        Dictionary<string, string> resolvedParameters = ResolveStepReferences(step.Parameters, context);
        string output = tool.Execute(resolvedParameters);
        return $"[Step {step.Id}] {output}";
    }
    ...
    private static HandlerResult CommandHandler(string input)
    {
        ...
        {
            return new HandlerResult("CommandHandler", "收到 /exit，程序退出。", true);
        }

        if (cmd.Equals("/help", StringComparison.OrdinalIgnoreCase))
        {
            return new HandlerResult("CommandHandler", BuildHelpText(), false);
        }

        if (cmd.Equals("/memory", StringComparison.OrdinalIgnoreCase))
        {
            string snapshot = Memory.GetMemorySnapshot();
            return new HandlerResult("CommandHandler", snapshot, false);
        }
    ...
    {
        Console.WriteLine("=== Agent CLI (Planner v2 + Memory Compression) ===");
    ...
    private static string BuildHelpText()
    {
        return string.Join(Environment.NewLine, new[]
        {
            ...
            "任务流程：User Input -> Retrieve Memory -> LLMPlanner -> Executor",
            "Memory 支持短期缓存、长期存储、定期压缩。"
        });
    }
}
...
{
    public ToolParameter(string name, string description, bool required)
    {
        Name = name;
...
internal sealed class WeatherTool : ITool
{
    ...
    {
        new ToolParameter("city", "城市", false),
        new ToolParameter("date", "日期", false)
    };
...
internal sealed class SummaryTool : ITool
{
    ...
    public IReadOnlyList<ToolParameter> Parameters => new List<ToolParameter>
    {
        new ToolParameter("text", "总结文本", true)
    };
...
internal sealed class ToolRegistry
{
    ...
    {
        return _tools.TryGetValue(toolName, out ITool? tool) ? tool : null;
    }
}
...
internal sealed class LLMPlanner
{
    public bool TryCreatePlan(string userTask, List<string> memoryContext, out Plan plan)
    {
        ...
        try
        {
            ...
            string json = CallMockLlm(prompt, userTask);
            Plan? parsed = ParsePlanJson(json);
            if (parsed is null)
            {
                return false;
            }
    ...
    private static string CallMockLlm(string prompt, string userTask)
    {
        _ = prompt;
        ...
        if (userTask.Contains("天气", StringComparison.Ordinal))
        {
            string city = userTask.Contains("上海", StringComparison.Ordinal) ? "上海" : userTask.Contains("北京", StringComparison.Ordinal) ? "北京" : "默认城市";
            string date = userTask.Contains("明天", StringComparison.Ordinal) ? "明天" : "今天";
            return JsonSerializer.Serialize(new
            {
                steps = new object[]
                {
                    new { id = 1, description = "查询天气", tool = "weather", parameters = new Dictionary<string, string> { ["city"] = city, ["date"] = date } }
                }
            });
        }

        throw new InvalidOperationException("mock llm planner 无法解析");
    }

    private static Plan? ParsePlanJson(string json)
    {
        using JsonDocument doc = JsonDocument.Parse(json);
        JsonElement root = doc.RootElement;

        if (!root.TryGetProperty("steps", out JsonElement stepsEl) || stepsEl.ValueKind != JsonValueKind.Array)
        {
            return null;
        }
        ...
        foreach (JsonElement item in stepsEl.EnumerateArray())
        {
            ...
            {
                steps.Add(new PlanStep(id, desc, tool, parameters));
            }
        }
...
internal sealed class MemoryManager
{
    ...
    public MemoryManager(
        ...
        LongTermMemory ltm,
        Retriever retriever,
        MemoryCompressionUnit compressor,
        int compressionTurnThreshold,
    ...
    public void ProcessTurn(string userInput, string agentOutput, IntentType intent)
    {
        ...
        {
            string type = DecideMemoryType(userInput, intent);
            _ltm.AddRecord(new MemoryRecord(Guid.NewGuid().ToString("N"), type, $"USER={userInput} | AGENT={agentOutput}", DateTime.UtcNow));
        }

        if (ShouldCompress(intent))
        {
            CompressAndStore();
        }
    ...
    {
        List<string> recentContext = _stm.GetContext();
        if (recentContext.Count == 0)
        {
            return;
        }

        MemoryCompressionResult result = _compressor.Compress(recentContext);

        if (!string.IsNullOrWhiteSpace(result.CompressedSummary))
        {
            if (ShouldStoreCompressedSummary(result.CompressedSummary))
            {
        ...
        foreach (string fact in result.ImportantFacts)
        {
            ...
            if (_ltm.ExistsContent(fact)) continue;
    ...
    private static bool ShouldStoreCompressedSummary(string summary)
    {
...
internal sealed class MemoryCompressionUnit
{
    ...
    {
        string prompt = BuildCompressionPrompt(recentContext);
        string json = CallMockCompressionLlm(prompt, recentContext);
        MemoryCompressionResult? parsed = ParseCompressionJson(json);
        return parsed ?? FallbackCompression(recentContext);
    }

    private static string BuildCompressionPrompt(List<string> recentContext)
    {
        return string.Join("\n", new[]
        {
            "你是一个 Memory Compression 模块。",
            "目标是压缩短期记忆（recent_context），保留关键信息并生成简洁摘要。",
            "要求：删除重复、无意义内容；抽取关键信息；summary 不超过 200 tokens；输出 JSON。",
            "输出格式：{\"compressed_summary\":\"...\",\"important_facts\":[\"...\"]}",
            "recent_context:",
            string.Join(" | ", recentContext)
        });
    }

    private static string CallMockCompressionLlm(string prompt, List<string> recentContext)
    {
        _ = prompt;
        ...
        List<string> facts = dedup
            .Where(x => x.Contains("喜欢", StringComparison.Ordinal)
                || x.Contains("天气", StringComparison.Ordinal)
                || x.Contains("总结", StringComparison.Ordinal)
                || x.Contains("生日", StringComparison.Ordinal))
            .Take(5)
            .ToList();
    ...
    private static MemoryCompressionResult? ParseCompressionJson(string json)
    {
        using JsonDocument doc = JsonDocument.Parse(json);
        JsonElement root = doc.RootElement;

        string summary = root.TryGetProperty("compressed_summary", out JsonElement sEl) ? sEl.GetString() ?? string.Empty : string.Empty;
        List<string> facts = new List<string>();

        if (root.TryGetProperty("important_facts", out JsonElement fEl) && fEl.ValueKind == JsonValueKind.Array)
        {
            foreach (JsonElement item in fEl.EnumerateArray())
            {
                facts.Add(item.GetString() ?? string.Empty);
            }
        }
    ...
    private static MemoryCompressionResult FallbackCompression(List<string> recentContext)
    {
        string summary = string.Join(" ; ", recentContext.Take(3));
        return new MemoryCompressionResult(summary, new List<string>());
    }
}
...
internal sealed class ShortTermMemory
{
    ...
    public int EstimateTokenSize()
    {
        int totalChars = _buffer.Sum(x => x.Length);
...
internal sealed class LongTermMemory
{
    ...
    public void AddRecord(MemoryRecord record)
    {
        ...
        {
            case "semantic":
                SemanticStore.Add(record);
                break;
            case "episodic":
                EpisodicStore.Add(record);
                break;
            case "procedural":
                ProceduralStore.Add(record);
                break;
            default:
                EpisodicStore.Add(record);
                break;
    ...
    public bool ExistsContent(string content)
    {
        return AllRecords().Any(r => r.Content.Equals(content, StringComparison.Ordinal));
    }

    public List<MemoryRecord> AllRecords()
    {
        return SemanticStore.Concat(EpisodicStore).Concat(ProceduralStore).ToList();
    }
}
...
internal sealed class Retriever
{
    public List<string> Retrieve(LongTermMemory ltm, string query, int topK)
    {
        ...
        if (keywords.Any())
        {
            records = records.Where(r => keywords.Any(k => r.Content.Contains(k, StringComparison.OrdinalIgnoreCase)));
        }
